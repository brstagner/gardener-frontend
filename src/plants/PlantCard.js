...
function PlantCard ({ plant, deletePlant }) {
  ...
  if (cardColors.length === 2)
    colorString = `linear-gradient(to right, ${cardColors[0]} 49%, ${cardColors[1]} 50%)`;
  if (cardColors.length === 3)
    colorString = `linear-gradient(to right, ${cardColors[0]} 32%, ${cardColors[1]} 34%, ${cardColors[1]} 65%, ${cardColors[2]} 67%)`;

  const months = Object.keys(plant.bloom_months).filter(
    (key) => plant.bloom_months[key] === true
  );
  ...
  return (
    <Box key={plant.plant_id} sx={{ margin: 2 }}>
      <Card sx={{ minWidth: 275, maxWidth: "30%" }} elevation={5}>
        ...
        <CardActions>
          <Button size="small" onClick={() => nav(`/plants/${plant.plant_id}`)}>
            edit
          </Button>
